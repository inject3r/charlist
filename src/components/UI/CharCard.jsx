...
const CharCard = ({ char, name }) => {
  ...
  const handleCopy = async () => {
    try {
      ...
      recentCopies = recentCopies.filter((item) => item !== char);
      recentCopies = [char, ...recentCopies].slice(0, 6);
  ...
  const getNameFontSize = () => {
    if (!name) return "";
    if (name.length <= 10) return "text-[8px] sm:text-[10px]";
    if (name.length <= 15) return "text-[7px] sm:text-[9px]";
    if (name.length <= 20) return "text-[6px] sm:text-[8px]";
    return "text-[5px] sm:text-[7px]";
  };

  const isLongName = name && name.length > 12;
